...
class DocumentMergerApp:
    ...
    def _build_ui(self):
        ...
        ttk.Label(header_frame, text="🗂  PDF & PPTX Merger (Win32)", style="Header.TLabel").pack(side="left")
        
        self.count_lbl = ttk.Label(header_frame, text="0 / 20 files", foreground='#8C94A6')
        self.count_lbl.pack(side="right")

        ttk.Label(main_container, 
                  text="Same format → merged in original format   |   Mixed → converted to PDF via Office COM",
    ...
    def _worker(self):
        ...
        try:
            ...
            abs_files = [os.path.abspath(f) for f in self.files]
            ...
            if mode == "pdf":
                PDF_Handler.merge_pdfs(abs_files, abs_out, self._update_status)
            elif mode == "pptx":
                PPT_Handler.merge_pptx(abs_files, abs_out, self._update_status)
            else:
                Handling_Mixed_Docs.merge_mixed(abs_files, abs_out, self._update_status)
