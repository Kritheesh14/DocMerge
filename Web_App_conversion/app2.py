...
def merge():
    ...
    paths = [
        str(sdir / safe)
        for safe, _ in entries
    ]

    exts = {
        Path(orig).suffix.lower()
        for _, orig in entries
    }
...
def merge_pptx(paths):
    ...
    for p in paths[1:]:
        ...
        for slide in src.slides:
            ...
            new_slide = base.slides.add_slide(
                base.slide_layouts[layout_idx]
            )

            for ph in new_slide.placeholders:
                ph._element.getparent().remove(
                    ph._element
                )

            for shape in slide.shapes:
                new_slide.shapes._spTree.insert(
                    2,
                    copy.deepcopy(shape._element)
                )
...
def pptx_to_pdf(pptx_path, out_dir):
    ...
    try:
        ...
        raise RuntimeError(
            "Native Windows PowerPoint conversion failed: "
            f"{str(e)}"
        )
    ...
    if not os.path.exists(output_pdf_path):
        raise FileNotFoundError(
            "PowerPoint completed execution, but the output "
            "PDF path is missing."
        )
